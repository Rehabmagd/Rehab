...
${AUTO_PASSWORD_TOGGELE}  id=save-family-toggle-auto-generated-password
${FAMILY_PASSWORD}     id=save-family-password
...
*** Keywords ***
Add Family
    [Arguments]  ${fristname}    ${lastname}    ${mobile}    ${email2}    ${fusername2}    ${fpassword}
    Wait Until Element Is Visible    ${FAMILY_SIDEBAR}      timeout=20s
    Click Element     ${FAMILY_SIDEBAR}
   Wait Until Element Is Visible    ${ADD_FAMILY}      timeout=10s
   ...
   Wait Until Element Is Visible    ${ADD_FIRSTNAME}    timeout=5s
    ...
    Execute Javascript    window.setTimeout(arguments[0], 5000)
    input text  ${FAMILY_EMAIL}    ${email2}
    Execute Javascript    window.setTimeout(arguments[0], 5000)
    input text  ${FAMILY_USERNAME}    ${fusername2}
    Execute Javascript    window.setTimeout(arguments[0], 5000)
    Click Element        ${imgebtn}
    Execute Javascript    window.setTimeout(arguments[0], 5000)
    Choose File      ${uploed}    C:/Users/ADMIN/Desktop/Cloudy.jpeg
    Execute Javascript    window.setTimeout(arguments[0], 5000)
    click button  ${AUTO_PASSWORD_TOGGELE}
    Execute Javascript    window.setTimeout(arguments[0], 5000)
    input text    ${FAMILY_PASSWORD}   ${fpassword}
    Execute Javascript    window.setTimeout(arguments[0], 5000)
